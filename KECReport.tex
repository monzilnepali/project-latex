...
\begin{figure}[h]
  \begin{center}
  \includegraphics[width=150mm,height=100mm]{images/feed forward.png}
  \caption{Feedforward Network Model(Source: edureka.co)}
  \end{center}
  \end{figure}
 
\subsection{Weight update for output layer}
 $$ *W\textsubscript{x} = W\textsubscript{x} - \eta (\frac{\delta Error}{\delta W\textsubscript{x} }) $$  
 
 \par where, \eta = learning rate  
 \par Let's take example to update w6,
 	$$ *W\textsubscript{6} = W\textsubscript{6} - \eta (\frac{\delta Error}{\delta W\textsubscript{6} }) $$  
 	
\par The derivation of the error function is evaluated by applying the chain rule as following:
$$ \frac{\delta Error}{\delta  W\textsubscript{6}} = \frac{\delta Error}{\delta prediction} * \frac{\delta prediction}{\delta  W\textsubscript{6}}$$ 	
$$ \frac{\delta Error}{\delta  W\textsubscript{6}} = \frac{\frac{1}{2}(prediction-actual)^2}{\delta prediction} * \frac{\delta(i\textsubscript{1} w\textsubscript{1} + i\textsubscript{2} w\textsubscript{2})w\textsubscript{5} + (i\textsubscript{1} w\textsubscript{3} + i\textsubscript{2} w\textsubscript{4})w\textsubscript{6}}{\delta w\textsubscript{6} } $$
$$ \frac{\delta Error}{\delta  W\textsubscript{6}} = 2 * \frac{1}{2}(prediction - actual)\frac{\delta(prediction-actual)}{\delta prediction} * (i\textsubscript{1} w\textsubscript{3} + i\textsubscript{2} w\textsubscript{4}) $$
$$ \frac{\delta Error}{\delta  W\textsubscript{6}} = (prediction - actual)*(h\textsubscript{2}) $$
$$ \frac{\delta Error}{\delta  W\textsubscript{6}} = \Delta h\textsubscript{2} $$

\par So to update w6 we can apply the follwing formula 
