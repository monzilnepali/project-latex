...
\par yn=f(xm*w(xm)n+bias)
\par where,
\par xm = input to neuron n
\par w(xm)n = weight of connection between input xm and neuron n
\par yn = output of neuron n
...
\begin{figure}[h]
 ...
 \par where, \eta = learning rate  
 \par Let's take example to update w6,
...
\par So to update w6 we can apply the following formula 
...
\pagebreak
 ...
 lets take example to update w1.Here partial derivative is done with respect to w1.
 
 
 $$ \frac{\partial Error}{\partial w\textsubscript{1}}= \frac{\partial Error}{\partial predication}*\frac{\partial predication}{\partial h1}* \frac{\partial h\textsubscript{1}}{\partial w\textsubscript{1}}$$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=\frac{\partial\frac{1}{2}*(predication-actual)}{\partial predication}*\frac{\partial (h1)w5+(h2)w6}{\partial h1}*\frac{\partial i1w1+i2w2}{\partial w1} $$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=2*\frac{1}{2}*(predication-actual)*\frac{\partial (predication-actual)}{predication}*(w5)*(i1)
$$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=(predication-actual)*(w5i1) $$
$$ \frac{\partial Error}{\partial w\textsubscript{1}}=\triangle w5i1 $$
  
 \par So to update w1 we can apply the following formula 
