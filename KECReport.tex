...
\makebox[7cm]{Abhishek Gurung\hfill [38987]}\\
\makebox[7cm]{Manjil Nepali  \hfill [39000]}\\
\makebox[7cm]{Sagar Maharjan \hfill [39012]}\\
\makebox[7cm]{Sovin Shrestha \hfill [39018]}
...
\newcommand{\defMonth}{September} %Defense Month- January, February, ...
...
\newcommand{\degSup}{Solution Architect} %Specify Designation of Supervisor for Major Project, use multiple lines (\\) if necessary
\newcommand{\external}{Prof. Dr. Sashidhar Ram Joshi} %Specify Name of External for Major Project (Required for Black Book)
\newcommand{\degExternal}{IOE Pulchowk Campus} %Specify Name of External for Major Project (Required for Black Book) , use multiple lines (\\) if necessary
...
\newline
1.Forward pass\newline
Calculation of output from neural network by propagating an input signal through each layer until the output layers outputs its values.

\begin{itemize}
\item Output of neurons:
$$y\textsubscript{n} = f(x\textsubscript{m}*w(x\textsubscript{m})n+bias)$$
\par where,
\par x\textsubscript{m} = input to neuron n
\par w(x\textsubscript{m})n = weight of connection between input x\textsubscript{m} and neuron n
\par y\textsubscript{n} = output of neuron n
\par f() = activation function like sigmoid, relu, tanh, etc.

\item Calculation of Error:
\par Calculation of the error for each output neuron using the squared error function and sum them to get the total error:
	$$ E\textsubscript{total} &=  \sum \frac{1}{2}(target-output)^2$$

\end{itemize}

2.Backward pass\newline
Error is calculated between the expected outputs and the outputs forward propagated from the network. These error are then propagated backward through the network from the output layer to the hidden layer, assigning blame for the error and updating weights as they go.
...
\begin{figure}[h]
  ...
  \textbf{Weight update for output layer} 
 $$ *W\textsubscript{x} = W\textsubscript{x} - \eta (\frac{\partial Error}{\delta W\textsubscript{x} }) $$  
 
 \par where, 
 $\eta$ = learning rate   
 \par Let's take example to update W\textsubscript{6},
 	$$ *W\textsubscript{6} = W\textsubscript{6} - \eta (\frac{\partial Error}{\delta W\textsubscript{6} }) $$  
\par The derivation of the error function is evaluated by applying the chain rule as following:
$$ \frac{\partial Error}{\delta  W\textsubscript{6}} = \frac{\partial Error}{\partial prediction} * \frac{\partial prediction}{\partial  W\textsubscript{6}}$$ 	
$$ \frac{\partial Error}{\partial  W\textsubscript{6}} = \frac{\frac{1}{2}(prediction-actual)^2}{\partial prediction} * \frac{\partial(i\textsubscript{1} w\textsubscript{1} + i\textsubscript{2} w\textsubscript{2})w\textsubscript{5} + (i\textsubscript{1} w\textsubscript{3} + i\textsubscript{2} w\textsubscript{4})w\textsubscript{6}}{\partial w\textsubscript{6} } $$
$$ \frac{\partial Error}{\partial  W\textsubscript{6}} = 2 * \frac{1}{2}(prediction - actual)\frac{\partial(prediction-actual)}{\delta prediction} * (i\textsubscript{1} w\textsubscript{3} + i\textsubscript{2} w\textsubscript{4}) $$
$$ \frac{\partial Error}{\delta  W\textsubscript{6}} = (prediction - actual)*(h\textsubscript{2}) $$
$$ \frac{\partial Error}{\delta  W\textsubscript{6}} = \Delta h\textsubscript{2} $$
\par So to update W\textsubscript{6} we can apply the following formula 
$$ *W\textsubscript{6} = W\textsubscript{6} - \eta \Delta h\textsubscript{2}  $$
 

 \textbf{Weight update for Hidden layer} 
 
 lets take example to update  w\textsubscript{1}.Here partial derivative is done with respect to w\textsubscript{1}.
 
 
 $$ \frac{\partial Error}{\partial w\textsubscript{1}}= \frac{\partial Error}{\partial predication}*\frac{\partial predication}{\partial h\textsubscript{1}}* \frac{\partial h\textsubscript{1}}{\partial w\textsubscript{1}}$$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=\frac{\partial\frac{1}{2}*(predication-actual)}{\partial predication}*\frac{\partial (h\textsubscript{1})w\textsubscript{5}+(h\textsubscript{2})w\textsubscript{6}}{\partial h\textsubscript{1}}*\frac{\partial i\textsubscript{1}w\textsubscript{1}+i\textsubscript{2}w\textsubscript{2}}{\partial w\textsubscript{1}} $$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=2*\frac{1}{2}*(predication-actual)*\frac{\partial (predication-actual)}{predication}*(w\textsubscript{5})*(i\textsubscript{1})
$$
$$\frac{\partial Error}{\partial w\textsubscript{1}}=(predication-actual)*(w\textsubscript{5}i\textsubscript{1}) $$
$$ \frac{\partial Error}{\partial w\textsubscript{1}}=\triangle w\textsubscript{5}i\textsubscript{1} $$
  
 \par So to update w\textsubscript{1} we can apply the following formula 
$$ *W\textsubscript{1} = W\textsubscript{1} - \eta \Delta i\textsubscript{1} w\textsubscript{5}  $$
...
\includegraphics[width=160mm,height=180mm]{images/usecase1.jpg}
...
For this we trained our neural Network with stochastic gradient descent for different value of learning rate and Activation function such as sigmoid, relu, tanh. We used cross validation test method to determine the performance of model on different parameters and  we conclude that sigmoid activation function and learning rate=0.05 is optimal for our neural Network model as we got around 95\%  accuracy while increasing or decreasing learning rate values reduce the accuracy of model.


\includegraphics[width=170mm,height=110mm]{comparison/activation.png}
...
Our project has got a lot of potential but for now due to limitation to our knowledge and time, we have proposed an able system to predict provided diseases only.
